...
/**
 * FollowersScreen - Displays users who follow the profile owner
 */
export const FollowersScreen = () => {
  const [refreshing, setRefreshing] = useState(false);
  const [loadingUserIds, setLoadingUserIds] = useState<Set<string>>(new Set());
  const navigation = useNavigation<MainScreenNavigationProp<'Followers'>>();
  const route = useRoute<MainScreenRouteProp<'Followers'>>();
  ...
  const {followerUsers, loading, refetch, loadMore, hasMore} = useFollowerUsers(
    profileUserId,
    20,
    0,
  );
   ...
   */
  const getUserFromFollower = (follower: IUserFollowing): IUser | null => {
    return (follower.follower as IUser) || null;
  };
  ...
  /**
   * Render empty state when no follower users exist
   */
    ...
    return (
      <View style={styles.emptyState}>
        <Body color={colors.neutral.grey} align="center">
          {t('screens.followers.no_followers')}
        </Body>
      </View>
    );
  }, [loading, t]);
  ...
  const renderUserItem = useCallback(
    ({item}: {item: IUserFollowing}) => {
      const user = getUserFromFollower(item);
      if (!user) {
        return null;
      }
  ...
  if (loading && followerUsers.length === 0) {
    return (
      <View style={styles.container}>
        <TopHeaderBar
          title={t('screens.followers.followers')}
          showBackButton
